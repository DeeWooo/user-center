 ...
 **/
@Deprecated
public enum AuthorizationClient {
    /**
     * 用户端
     */
    ...
    AuthorizationClient(String type, String name) {
        this.type = type;
        this.name = name;
    }

    public static final Map<String,AuthorizationClient> AUTHORIZATIONCLIENT_MAP = Maps.newHashMap();

    static {
        Arrays.stream(AuthorizationClient.values()).forEach(scope -> {
            AUTHORIZATIONCLIENT_MAP.put(scope.getType(),scope);
        });
    }
